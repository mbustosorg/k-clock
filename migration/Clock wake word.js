...
* Max 12ma per color (36ma per pixel)
1 panel: 1.22A
6 panels limited to 1.8A for USB, + 190ma for PB: Global brightness 20% (222 Pixels)
*/

var modeCount = 6
export var mode
export function sliderMode(_v) {
  mode = floor((_v * .999) * modeCount)
}

var modes = array(modeCount)
modes[0] = (i, x, y, z) => hsv(0, 1, radius)

// Rotating lines
modes[1] = (i, x, y, z) => {
  hsv(radius/3 + t2, 1, near((x+y)/2 + .5, .6, .15))
}

modes[2] = (i, x, y, z) => hsv(t2 / 2, 1, near(radius, t1e, .6) * near(phi, t1e * 8 * PI % PI2 - PI, .6))

modes[3] = function(index, x, y, z) {
  x += (z*4 + .5)
  v = near((x + y)/6.6, t1e * 1.2, 42/222)
  hsv(.04, .9, v * v * v)
}

modes[4] = function(index, x, y, z) {
  irisCenterX = cos(t1 * PI2) / 4
  irisCenterY = sin(t1 * PI2) / 4
  v = 2 * near(irisCenterX, x, .7) * near(irisCenterY, y, .7)
  hsv(.6, .7, 1 - v)
}

modes[5] = function(index, x, y, z) {
  i = index % 37
  var s1 = i == 6 ||
          i == 5 ||
          i == 10 ||
  ...
  v = s1 || s2 || s3
  hsv(0,0,0)
  if (s1) hsv(0, 1, .051 * (t1 < .5))
  if (s2) hsv(.66, 1, .051 * ((t1 + .2) % 1 < .8))
//  hsv(s1/2 + s2/4 + s3/3, .7, v/8)
  if (!v) hsv(.98, 1, .9)
}


var t1, t1e, t2
function beforeRenderWakeWord(delta) {
  //resetTransform()
  //translate3D(-.5, -.5, 0)
  if (mode == 1) rotate(t1*PI2)
  t1 = time(.01)
  t1e = 1.5 * t1 - .25
  t2 = 0//time(1)
}
...
export function render3D(index, x, y, z) {
  dist = pow(pow(x - 0.5, 2) + pow(y - 0.5, 2), 0.5) / .15
  //phi = atan2(y, x)
  radius = wave(time(0.01)) * 3
  hsv(0, 1, (1 - dist) / radius)
}
...
var renderBackground = (index, x, y) => {}
