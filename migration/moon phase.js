/*
  ...
  Displays the current moon phase on a round LED matrix.
  Use the slider to select the phase (0.0 and 1.0 = New Moon, 0.5 = Full Moon)
...
// UI Controls
export var phase = 0.5      // 0 to 1, where 0=new, 0.25=first quarter, 0.5=full, 0.75=last quarter
export var radius = 0.8     // Moon size: 0.1 to 1.0
export var moonIndex = 4    // 0-7: moon phase index
var prevMoonIndex = -1

export function sliderRadius(v) {
  radius = 0.1 + v * 0.9  // Map 0-1 slider to 0.1-1.0 radius
}

export function sliderMoonIndex(v) {
  moonIndex = floor(v * 8)  // Map 0-1 to 0-7
  if (moonIndex > 7) moonIndex = 7
}

export function beforeRenderMoon(delta) {
  if (prevMoonIndex != moonIndex) {
...
// Moon appearance settings
var moonBrightness = 0.9
var moonColor = [0.95, 0.92, 0.85]  // Warm white/cream color
var skyBrightness = 0.05
var skyColor = [0.1, 0.15, 0.3]     // Deep blue

// Calculate if a point should be lit based on moon phase
// x, y are normalized coordinates from -1 to 1 (center at origin)
function isLitAtPhase(x, y, phase, radius) {
  // Check if point is within the moon circle
  var distFromCenter = sqrt(x * x + y * y)
  if (distFromCenter > radius) return 0  // Outside moon circle
  
  // Phase 0.0 = New Moon (completely dark)
  // Phase 0.125 = Waxing Crescent (sliver on RIGHT side)
  // Phase 0.25 = First Quarter (RIGHT half lit)
  // Phase 0.375 = Waxing Gibbous (mostly lit, small dark on LEFT)
  // Phase 0.5 = Full Moon (completely lit)
  // Phase 0.625 = Waning Gibbous (mostly lit, small dark on RIGHT)
  // Phase 0.75 = Last Quarter (LEFT half lit)
  // Phase 0.875 = Waning Crescent (sliver on LEFT side)
  // Phase 1.0 = New Moon (completely dark)
  ...
  // For waxing (0 to 0.5): RIGHT side gets lit
  // For waning (0.5 to 1): LEFT side gets lit
  
  var isWaxing = phase < 0.5
  
  // Calculate how much is illuminated (0 to 1)
  var illum = isWaxing ? (phase * 2) : (2 - phase * 2)
  
  // Terminator position calculation:
  // For WAXING: terminator moves from RIGHT to LEFT as moon fills
  //   illum=0: termX = +radius (right edge)
  //   illum=1: termX = -radius (left edge)
  // For WANING: terminator moves from LEFT to RIGHT as moon empties
  //   illum=1: termX = -radius (left edge)
  //   illum=0: termX = +radius (right edge)
  
  var termX
  if (isWaxing) {
    termX = radius - (illum * 2 * radius)  // +radius -> -radius
  } else {
    termX = -radius + (illum * 2 * radius)  // -radius -> +radius
  }
  
  // Add curvature for realistic moon phases
  // The terminator follows an ellipse shape
  // For crescents: narrow ellipse (concave from lit side)
  // For gibbous: wide ellipse (convex into dark side)
  // For quarters: straight line (no curve)
  
  var isQuarter = abs(illum - 0.5) < 0.02  // Within 2% of half
  
  if (!isQuarter && illum > 0.01 && illum < 0.99) {  // Not new, full, or quarter moon
    var ySquared = y * y
    if (ySquared < radius * radius) {
      var isGibbous = illum > 0.5
      
      // Calculate the ellipse semi-major axis (horizontal width of the curve)
      var a  // semi-major axis
      if (isGibbous) {
        // Gibbous: ellipse width is based on dark portion
        a = (1 - illum) * radius
      } else {
        // Crescent: ellipse width is based on lit portion
        a = illum * radius
      }
      
      if (a > 0.01) {  // Avoid division by zero
        // Ellipse equation: x²/a² + y²/b² = 1, where b = radius
        // Solve for x: x = ±a * sqrt(1 - y²/radius²)
        var ellipseX = a * sqrt(1 - ySquared / (radius * radius))
        
        // Apply the curve to the terminator position
        if (isWaxing) {
          if (isGibbous) {
            // Waxing gibbous: curve goes left (negative)
            termX = termX - ellipseX
          } else {
            // Waxing crescent: curve goes right (positive)
            termX = termX + ellipseX
          }
        } else {
          if (isGibbous) {
            // Waning gibbous: curve goes right (positive)
            termX = termX + ellipseX
          } else {
            // Waning crescent: curve goes left (negative)
            termX = termX - ellipseX
          }
        }
      }
  ...
  if (isWaxing) {
    // Waxing: light on RIGHT, so lit if x > termX
    return x > termX ? 1 : 0
  } else {
    // Waning: light on LEFT, so lit if x < termX
    return x < termX ? 1 : 0
  }
}

export function renderMoon(index, x, y) {
  // Convert from Pixelblaze's 0-1 coordinates to centered -1 to 1 space
  // x and y are normalized 0-1, convert to -1 to 1 centered at origin
  var nx = (x - 0.5) * 2
  var ny = (y - 0.5) * 2
  
  // Check if pixel is within moon and lit
  var lit = isLitAtPhase(nx, ny, phase, radius)
  
  // Blend between sky and moon color
  var r, g, b
  if (lit > 0) {
    r = moonColor[0] * moonBrightness
    g = moonColor[1] * moonBrightness  
    b = moonColor[2] * moonBrightness
  } else {
    r = skyColor[0] * skyBrightness
    g = skyColor[1] * skyBrightness
    b = skyColor[2] * skyBrightness
  }
  
  // Add subtle variation for realism
  var noise = (sin(x * 31.4 + y * 27.8) * 0.5 + 0.5) * 0.1
  
  rgb(r + noise * lit, g + noise * lit, b + noise * lit)
}

var beforeRenderBackground = (delta) => beforeRenderMoon(delta)
var renderBackground = (index, x, y) => renderMoon(index, x, y)
