...
export function beforeRenderTwinkles(delta) {
  t1 = time(.50) * PI2 
  t2 = time(.15) * PI2 // 3.33 times faster than t1
}

export function render2DTwinkles(index, x, y) {
  /*
  ...
  */
  h = sin(index / 1 + PI2 * sin(index / 1 + t1)) * .025 + 0.025

  // It takes some minor algebra to work out sin() vs wave(). See the docs.
  v = wave(index /2 / PI2 + sin(index /2 + t2))
  v = v * v * v * v * v // Gamma correction
...
var beforeRenderBackground = (delta) => beforeRenderTwinkles(delta)
var renderBackground = (index, x, y) => render2DTwinkles(index, x, y)
