var rgbGradient = [
  ...
  0.2,    1, 0, 0, //position start, rgb(0,0,0)
  0.9, 1, .6, 0, //position 75%, rgb(1,0,1)
  1,    1, .8, .3  // position end, rgb(0,1,1)
]
...
export function beforeRenderFire(delta) {
  t1 = time(.1)
  t2 = time(.07)
  t3 = time(.133)
  ...
  noiseYTime = time(1.6) * 256
  mode = floor(time(.3) * modes.length)
  mode = 3
  modeFn = modes[mode]
}

var s = 2
export function renderFire(index, x, y) {
  // Apply translate(-.5, 0.7) + scale(2, 2) manually
  var xf = (x - 0.5) * s
  var yf = (y + 0.7) * s
  //xf = xf + sin(((yf - 0.5) / s + t2 + wave(t3)) * PI2) * .15 * (s - yf)
  v = min(1, max(0, modeFn(xf, yf / 2 + noiseYTime, noiseTime) * 2 * (triangle(0.5 + xf / s) * 1 - 0)) * yf / s)
  paint(v, v * v)
}

var beforeRenderBackground = (delta) => beforeRenderFire(delta)
var renderBackground = (index, x, y) => renderFire(index, x, y)
