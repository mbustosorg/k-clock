...
// current settings
export var numPoints = 4;
export var speed = 0.05;
export var splatter = 1.75; 
export var edge = 0.082;

// UI
export function sliderNumberOfPoints(v) {
  var n;
  n = floor(2 + (v * (maxPoints - 2)));
  if (n != numPoints) {
    numPoints = n;
...
function initPoints() {
  for (var i = 0; i < numPoints; i++) {
    ...
    b[0] = random(1) - 0.5;   // x position 
    b[1] = random(1) - 0.5;   // y position
...
// corners for speed.  We'll catch it in a frame or two anyway
    if (b[0] < -boxSize) { b[0] = -boxSize; b[2] = -b[2]; continue; } 
    if (b[1] < -boxSize) { b[1] = -boxSize; b[3] = -b[3]; continue; }
    if (b[0] > boxSize) { b[0] = boxSize; b[2] = -b[2]; continue; }
    if (b[1] > boxSize) { b[1] = boxSize; b[3] = -b[3]; continue; }
  }
}
...
export function beforeRenderMeatballs(delta) {
  bounce();
}
...
// minimum distances.
export function renderMeatballs(index,x,y) {
  var minDistance,i,r,h,v;
  // Apply translate(-0.5,-0.5) + rotate(PI/4) manually
  var cx = x - 0.5
  var cy = y - 0.5
  var rx = cx * cos45 - cy * cos45
  var ry = cx * cos45 + cy * cos45
  ...
  for (i = 0; i < numPoints; i++) {
    r = minDistance * hypot(Points[i][0] - rx, Points[i][1] - ry) * splatter;
  ...
  if (minDistance >= edge) {
    ...
    hsv(edge - minDistance, 1, 1.2 - (wave(5 * minDistance)));
  }
}

var beforeRenderBackground = (delta) => beforeRenderMeatballs(delta)
var renderBackground = (index, x, y) => renderMeatballs(index, x, y)
