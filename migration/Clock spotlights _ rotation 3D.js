...
export function spotlightBeforeRender(delta) {
  // We could just use sin(time()) to output -1..1, but that's almost too smooth
  t1 = 2 * triangle(time(.3 / speed)) - 1
  t2 = 2 * triangle(time(.4 / speed)) - 1
  t3 = 2 * triangle(time(.5 / speed)) - 1
  t4 = time(.2 / speed)
...
export function spotlightRender2D(index, x, y) {
...
var beforeRenderBackground = (delta) => spotlightBeforeRender(delta)
var renderBackground = (index, x, y) => spotlightRender2D(index, x, y)
