...
speed = .01  // How fast the spheres travel through 3D space

export function beforeRenderCube(delta) {
  t0 = (wave(time(0.2 / speed)) - 0.5) * 2
  t1 = time(.01 / speed) + 0.5 * t0// x offset
  t2 = time(.01 / speed) - 0.4 * t0   // y offset
  t3 = time(.01 / speed) + 0.0 // z offset

  // Oscillate the scale coefficient of space between 0.25 and 0.75
  scale = (2 + wave(time(.1))) / 2
}

export function cubeRender3D(index, x, y, z) {
...
var beforeRenderBackground = (delta) => beforeRenderCube(delta)
var renderBackground = (index, x, y) => cubeRender3D(index, x, y, 0)
